...
test('TokiMate core modules do not pin stale cache versions',()=>{
  ...
  assert.match(nav,/app\.js\?v=076b5/);
  assert.match(extras,/APP_VERSION='v0\.7\.6'/);
  assert.match(extras,/attendance-core\.mjs\?v=076c3/);
  assert.match(extras,/salary-basis\.mjs\?v=076/);
  assert.match(extras,/payroll-review\.mjs\?v=076c3/);
  assert.match(extras,/scheduled-hours\.mjs\?v=076c3/);
});

test('TokiMate PWA precache uses current release URLs',()=>{
  const sw=read('zangyo36/sw.js');
  assert.match(sw,/CACHE_NAME='zangyo36-v0\.7\.6-work-type-order6'/);
  for(const asset of ['navigation.js','salary-basis.mjs']){
    assert.match(sw,new RegExp(`${asset.replace('.','\\.')}\\?v=${VERSION}`));
  }
  assert.match(sw,/app\.js\?v=076b5/);
  for(const asset of ['logic.mjs','attendance-core.mjs','payroll-review.mjs','scheduled-hours.mjs'])assert.match(sw,new RegExp(`${asset.replace('.','\\.')}\\?v=076c3`));
  assert.match(sw,/extras\.mjs\?v=076m7/);
