...
function repairCharacterImage(img){
  ...
  if(!src.includes('/characters-v066/'))img.src=`${CHARACTER_ROOT}${id}.jpg?v=075`;
}
...
document.addEventListener('error',e=>{const img=e.target;if(!(img instanceof HTMLImageElement))return;const id=img.dataset.characterId||characterIdFromSrc(img.getAttribute('src')||'');if(!id||img.dataset.characterFallback==='1')return;img.dataset.characterFallback='1';img.src=`./characters/${id}.jpg?v=075`;},true);
applyBranding();
repairAllCharacters();

import('./navigation.js?v=075').catch(e=>console.error('navigation fallback load failed',e));
import('./attendance-core.mjs?v=075').catch(e=>console.error('attendance core load failed',e));
import('./character-motion.mjs?v=068').catch(e=>console.error('character motion load failed',e));
import('./salary-basis.mjs?v=075').catch(e=>console.error('salary basis load failed',e));