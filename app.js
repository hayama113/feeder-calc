...
function renderResult(){
  ...
  setHtmlIfExists('resMinJudge', branchJudgeHtml((r.requiredMinCableCorrectedAmpacity || 0) >= (r.requiredBreaker || 0) && (r.requiredMinCableVoltageDropPercent || 0) <= DROP_LIMIT_PERCENT));
  ...
  setHtmlIfExists('resRecJudge', branchJudgeHtml((r.recommendedCableCorrectedAmpacity || 0) >= (r.adoptedBreaker || 0) && (r.recommendedCableVoltageDropPercent || 0) <= DROP_LIMIT_PERCENT));
...
function renderDocs(){
  ...
  DOC_GROUPS.forEach(group=>{
    ...
    const groupBody = document.createElement('div'); groupBody.className = 'stack top-gap doc-group-body'; groupBody.id = `doc-group-${group.key}`;
    let visibleCount = 0;
    group.items.forEach(item=>{
      ...
      itemButton.setAttribute('aria-controls', `doc-item-${itemKey.replace(/[^a-zA-Z0-9_-]/g,'-')}`);
      ...
      const itemBody = document.createElement('div'); itemBody.className = 'top-gap doc-item-body'; itemBody.id = itemButton.getAttribute('aria-controls');
    ...
    groupTitle.innerHTML = `<span class="doc-toggle-label"><span class="doc-chevron">${makeDocChevron(groupOpen)}</span><span>${escapeHtml(group.title)}</span></span><span class="doc-count">${visibleCount}件</span>`;
