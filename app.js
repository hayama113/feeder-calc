...
const DEFAULT_UI = {
  ...
  wiringLength:'', existingBreaker:'', breakerMarginRatio:'0.8', projectName:'', projectRemarks:'', loadCount:'',
...
function createCalcTab(label){
  return { id: makeId('calc-tab'), label, state: createFreshState() };
}

function initCalcTabs(){
  calcTabs = [createCalcTab('計算1')];
  activeCalcTabId = calcTabs[0].id;
  state = calcTabs[0].state;
}

function activeCalcTab(){
  return calcTabs.find(tab => tab.id === activeCalcTabId) || calcTabs[0] || null;
}

function renumberCalcTabs(){
  calcTabs.forEach((tab,index) => { tab.label = `計算${index + 1}`; });
}
...
function renderCalcTabs(){
  ...
  calcTabs.forEach(tab => {
    ...
    btn.type = 'button';
    btn.className = `calc-tab ${tab.id === activeCalcTabId ? 'active' : ''}`;
    btn.dataset.tabId = tab.id;
    const closeHtml = calcTabs.length > 1 ? '<span class="calc-tab-close" aria-label="この計算タブを削除">×</span>' : '';
    btn.innerHTML = `<span class="calc-tab-label">${escapeHtml(tab.label)}</span>${closeHtml}`;
    btn.addEventListener('click', event => {
      if (event.target.closest('.calc-tab-close')) {
        deleteCalcTab(tab.id);
        return;
      }
  ...
  const addBtn = $('addCalcTabBtn');
  if (addBtn) addBtn.disabled = calcTabs.length >= MAX_CALC_TABS;
}

function switchCalcTab(tabId){
  const tab = calcTabs.find(v => v.id === tabId);
  if (!tab) return;
  activeCalcTabId = tab.id;
  state = tab.state;
  renderAll();
  validateRealtime();
}
...
function addCalcTab(){
  if (calcTabs.length >= MAX_CALC_TABS) return showToast(`計算タブは最大${MAX_CALC_TABS}件です。`);
  const tab = createCalcTab(`計算${calcTabs.length + 1}`);
  calcTabs.push(tab);
  activeCalcTabId = tab.id;
  state = tab.state;
  renderAll();
  showToast(`${tab.label}を追加しました。`);
}

function deleteCalcTab(tabId){
  if (calcTabs.length <= 1) return showToast('計算タブは1件以上必要です。');
  const index = calcTabs.findIndex(v => v.id === tabId);
  if (index < 0) return;
  const targetLabel = calcTabs[index].label;
  if (!confirm(`${targetLabel}を削除しますか？入力中の内容は破棄されます。`)) return;
  calcTabs.splice(index, 1);
  renumberCalcTabs();
  const next = calcTabs[Math.max(0, index - 1)] || calcTabs[0];
  activeCalcTabId = next.id;
  state = next.state;
  renderAll();
  showToast(`${targetLabel}を削除しました。`);
}

function clearCurrentCalcTab(){
  const tab = activeCalcTab();
  if (!tab) return;
  if (!confirm(`${tab.label}の入力値と計算結果をクリアしますか？`)) return;
  tab.state = createFreshState();
  state = tab.state;
  renderAll();
  validateRealtime();
  showToast(`${tab.label}をクリアしました。`);
}
...
function candidateFromSize(type, size, breakerCurrent, preferManualAmpacity=false){
  ...
  return {size:Number(size), info, basis, corrected, drop, currentOk:corrected>=breakerCurrent, dropOk:drop.dropPercent<=DROP_LIMIT_PERCENT};
}
...
function rootMemo(){
  ...
  notes.push(state.calcMode === 'existing' ? '既設条件を優先した計算です。' : `総負荷÷${state.breakerMarginRatio || '0.8'}を基準に推奨開閉器を選定しています。`);
  notes.push('技術資料タブの基準条件を参照してください。');
  if (state.cableSizingMode === 'manual') notes.push('任意ケーブルサイズを手動指定して判定しています。');
  if (state.ampacityMode === 'manual') notes.push('ケーブル許容電流は手動入力値を採用しています。');
  return notes;
}
...
function missingFields(){
  ...
  if (state.cableSizingMode === 'manual') {
    ...
    pushIf(!state.baseAmpacity, 'ケーブル許容電流を入力してください。', 'baseAmpacity');
  }
...
function calculate(){
  ...
  const failReasons = [];
  if (!currentOk) failReasons.push(`指定ケーブル許容電流不足（推奨開閉器 ${formatNumber(recommendedBreaker,1)}A / 指定ケーブル補正後許容 ${formatNumber(corrected,1)}A）`);
  ...
  const result = {
    ...
    projectName:state.projectName || '', projectRemarks:state.projectRemarks || '',
    ...
    voltageDropV:drop.dropV, voltageDropPercent:drop.dropPercent,
  ...
  historyItems.unshift({id:makeId('history'), title:`${activeCalcTab()?.label || '計算'}_${state.calculationType === 'power' ? '低圧動力幹線計算' : '低圧電灯幹線計算'}_${new Date().toLocaleString('sv-SE').replace(/[: ]/g,'_')}`, memo:'', savedAt:new Date().toISOString(), input:structuredClone(state), result:structuredClone(result), calculationMode:state.calcMode, versions:structuredClone(VERSIONS)});
...
function renderResult(){
  ...
  if ($('resMassTotal')) $('resMassTotal').textContent = formatNumber(r.massTotalKg,2);
  ...
  if ($('printTotalLoadCount')) $('printTotalLoadCount').textContent = String(state.loads.length || 0);
  if ($('printTotalKW')) $('printTotalKW').textContent = formatNumber(r.totalKW,3);
  if ($('printTotalKVA')) $('printTotalKVA').textContent = formatNumber(r.totalKVA,3);
  if ($('printTotalCurrent')) $('printTotalCurrent').textContent = formatNumber(r.totalCurrent,2);
  if ($('printRecommendedBreaker')) $('printRecommendedBreaker').textContent = r.adoptedBreaker ? `${r.adoptedBreaker}A` : '-';
  if ($('printAdoptedCable')) $('printAdoptedCable').textContent = cableLabel(r.cableType, r.cableSize);
  if ($('printMassTotal')) $('printMassTotal').textContent = formatNumber(r.massTotalKg,2);
  if ($('printDropPct')) $('printDropPct').textContent = formatNumber(r.voltageDropPercent,2);
  ...
  r.loadDetails.forEach((item,index)=>{
    ...
    card.innerHTML = `<div class="row"><strong>負荷${index+1}</strong><span class="muted">${escapeHtml(item.name || '-')}</span></div><div class="readonly-grid top-gap"><div><span>入力方式</span><strong>${escapeHtml(item.inputType || '-')}</strong></div><div><span>負荷値</span><strong>${escapeHtml(item.value || '-')} ${escapeHtml(item.inputType || '')}</strong></div><div><span>換算電流 [A]</span><strong>${formatNumber(item.current,2)}</strong></div><div><span>換算容量 [kW]</span><strong>${formatNumber(item.kw,3)}</strong></div><div><span>換算容量 [kVA]</span><strong>${formatNumber(item.kva,3)}</strong></div></div>`;
...
function renderLoadCards(){
  ...
  state.loads.forEach((load,index)=>{
    ...
    card.innerHTML = `<div class="row"><strong>${escapeHtml(load.title)}</strong><span class="muted">${index+1} / ${state.loads.length}</span></div><div class="grid two top-gap"><label><span>負荷名称</span><input id="load-name-${load.id}" type="text" value="${escapeHtml(load.name)}" /></label><label><span>入力方式</span><select id="load-type-${load.id}"></select></label><label><span>負荷値</span><input id="load-value-${load.id}" type="number" inputmode="decimal" value="${load.value === '' ? '' : escapeHtml(load.value)}" /></label><div class="readonly-box compact-box"><span>入力方式</span><strong id="load-type-label-${load.id}">${escapeHtml(load.inputType || '-')}</strong></div></div><div class="readonly-grid top-gap"><div><span>換算電流 [A]</span><strong id="load-current-${load.id}">${formatNumber(currentForLoad(load),2)}</strong></div><div><span>換算容量 [kW]</span><strong id="load-kw-${load.id}">${formatNumber(kwForLoad(load),3)}</strong></div><div><span>換算容量 [kVA]</span><strong id="load-kva-${load.id}">${formatNumber(kvaForLoad(load),3)}</strong></div></div>`;
    ...
    typeSelect.addEventListener('change',()=>{ load.inputType = typeSelect.value; updateLoadDerivedValues(load); validateRealtime(); persistState(); });
    wireNumericInput(valueInput,()=>{ load.value = valueInput.value; updateLoadDerivedValues(load); validateRealtime(); persistState(); });
  });
...
function buildFormulaCoef(){ return textDoc('許容電流補正の基本式です。', [['補正後許容電流','ケーブル許容電流 × 温度補正 × 条数補正 × 敷設方法補正 × 敷設条件補正'],['温度補正','20〜50℃の係数テーブルを使用'],['条数補正','1〜6条の係数テーブルを使用'],['敷設方法補正','気中・ラック・配管の選択値を使用'],['敷設条件補正','一般・密集・日射ありの選択値を使用'],['注意','密集・日射・管路・ラック等の現場条件を反映して確認してください。']]); }
function buildMainBreakerDoc(){ return textDoc('主幹選定の考え方です。', [['必要最小開閉器','合計電流以上となる最小の標準開閉器です。'],['推奨開閉器','総負荷÷開閉器裕度設定を基準に標準開閉器へ丸めます。'],['裕度','開閉器裕度[%] = (推奨開閉器 - 合計電流) ÷ 推奨開閉器 × 100'],['注意','始動電流・需要率・保護協調・短絡容量は別途確認してください。']]); }
...
function buildSaveCard(item, isHistory){
  ...
  if (isHistory) card.querySelector('.js-save-history').onclick = ()=>{ const copied = structuredClone(item); copied.id = makeId('saved'); copied.title = `${item.title || '履歴'}_保存`; copied.savedAt = new Date().toISOString(); upsertSavedItem(copied); persistState(); renderSaved(); showToast('保存済みに追加しました。'); };
  else {
...
function openSavedItem(item){
  const nextState = migrateStateShape(item.input);
  nextState.lastResult = structuredClone(item.result);
  const tab = activeCalcTab();
  if (tab) tab.state = nextState;
  state = nextState;
  persistState();
  renderAll();
  switchScreen('calc');
...
function renderAmpacitySection(){
  const corrected = correctedAmpacity(), breakdown = getCorrectionBreakdown();
  ...
  if (state.ampacityMode === 'auto') badge.textContent = '自動'; else if (state.ampacityMode === 'manual') { badge.textContent = '手動'; badge.classList.add('manual'); } else { badge.textContent = '未設定'; badge.classList.add('none'); }
}
...
function bindEvents(){
  ...
  $('addCalcTabBtn')?.addEventListener('click', addCalcTab);
  $('clearCurrentCalcBtn')?.addEventListener('click', clearCurrentCalcTab);
  ...
  $('powerSystem').addEventListener('change',()=>{ state.powerSystem = $('powerSystem').value; persistState(); renderLoadCards(); validateRealtime(); });
  $('voltage').addEventListener('change',()=>{ state.voltage = $('voltage').value; persistState(); renderLoadCards(); validateRealtime(); });
  $('powerFactor').addEventListener('change',()=>{ state.powerFactor = $('powerFactor').value; persistState(); renderLoadCards(); validateRealtime(); });
  $('efficiency').addEventListener('change',()=>{ state.efficiency = $('efficiency').value; persistState(); renderLoadCards(); validateRealtime(); });
  $('existingBreaker').addEventListener('change',()=>{ state.existingBreaker = $('existingBreaker').value; persistState(); validateRealtime(); });
  $('breakerMarginRatio').addEventListener('change',()=>{ state.breakerMarginRatio = $('breakerMarginRatio').value || '0.8'; persistState(); validateRealtime(); });
...
function confirmSave(){
  ...
  const item = {id:makeId('saved'), title, memo:$('saveMemoInput').value.trim(), savedAt:new Date().toISOString(), input:structuredClone(state), result:structuredClone(state.lastResult), calculationMode:state.calcMode, versions:structuredClone(VERSIONS)};
...
function outputRows(){
  const r = state.lastResult;
  return [
    ['工事件名', state.projectName || ''], ['工事種別', state.calculationType === 'power' ? '低圧動力幹線計算' : '低圧電灯幹線計算'], ['提出先',''], ['図番',''], ['備考', state.projectRemarks || ''], ['計算方式', state.calcMode === 'existing' ? '既設開閉器指定' : '自動選定'], ['電源方式', state.powerSystem], ['電圧[V]', state.voltage], ['力率', state.powerFactor], ['効率', state.efficiency], ['配線長[m]', state.wiringLength], ['既設開閉器[A]', state.existingBreaker || ''], ['開閉器裕度設定', state.breakerMarginRatio || ''], ['必要最小開閉器[A]', r.requiredBreaker], ['推奨開閉器[A]', r.adoptedBreaker], ['必要最小ケーブルサイズ', cableLabel(r.requiredMinCableType, r.requiredMinCableSize)], ['自動推奨ケーブル', cableLabel(r.recommendedCableType, r.recommendedCableSize)], ['任意ケーブルサイズ', cableLabel(r.cableType, r.cableSize)], ['ケーブル許容電流[A]', r.baseAmpacity], ['補正後許容電流[A]', r.correctedAmpacity], ['合計容量[kW]', r.totalKW], ['合計容量[kVA]', r.totalKVA], ['合計電流[A]', r.totalCurrent], ['電圧降下[V]', r.voltageDropV], ['電圧降下[%]', r.voltageDropPercent], ['開閉器裕度[%]', r.breakerMarginPercent], ['容量裕度[kW]', r.capacityMarginKW], ['概算質量[kg/m]', r.massKgM], ['概算総質量[kg]', r.massTotalKg], ['参考ラック幅', r.rackWidth], ['良否判定', r.judgement], ['選定主因', r.mainFactor], ['選定根拠', r.reasons.join('、')], ['許容電流データ版', VERSIONS.ampacity], ['外径・質量データ版', VERSIONS.physical], ['帳票様式版', VERSIONS.form]
  ];
}
...
function init(){ ensureDocsUpgraded(); initCalcTabs(); bindEvents(); applySettingsValues(); renderAll(); switchScreen('calc'); switchSavedTab('history'); runDisclaimer(); registerSW(); }
