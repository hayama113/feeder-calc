...
const DOC_GROUPS = [
  {key:'calc_tools', title:'任意計算', items:[
    {id:'tech_calc_tools', title:'任意計算フォーム', searchable:true, build:buildTechCalcToolsDoc}
  ]},
...
function normalizeCalculationTypeValue(value){
  return value === 'lighting' ? 'lighting' : 'power';
}
...
function renderConduitWireRows(){
  ...
  conduitWireItems.forEach(item=>{
    ...
    row.querySelector('.js-conduit-wire-type').addEventListener('change',e=>{item.type=e.target.value;item.size=String(conduitWireSizes(item.type)[0]||'');renderConduitWireRows();calculateConduitSizing();});
    row.querySelector('.js-conduit-wire-size').addEventListener('change',e=>{item.size=e.target.value;renderConduitWireRows();calculateConduitSizing();});
    row.querySelector('.js-conduit-wire-count').addEventListener('input',e=>{item.count=e.target.value;calculateConduitSizing();});
    row.querySelector('.js-conduit-wire-remove').addEventListener('click',()=>{conduitWireItems=conduitWireItems.filter(v=>v.id!==item.id);renderConduitWireRows();calculateConduitSizing();});
...
function calculateConduitSizing(){
  ...
  if(selectedResult){const ok=selectedResult.fill<=limit;$('conduitJudgement').textContent=ok?'OK':'NG';$('conduitJudgement').className=`judgement ${ok?'good':'bad'}`;}
  const displayPipe=selectedResult||recommended;
  $('conduitActualFillLabel').textContent=selectedResult?'選択配管の占有率':'推奨配管の占有率';
  $('conduitPipeAreaLabel').textContent=selectedResult?'選択管内面積':'推奨管内面積';
  $('conduitActualFill').textContent=displayPipe?`${formatNumber(displayPipe.fill,1)}%`:'-'; $('conduitLimitResult').textContent=`${limit}%以下`; $('conduitMinimum').textContent=minimum?`${family} ${minimum.nominal}`:'該当なし'; $('conduitRecommended').textContent=recommended?`${family} ${recommended.nominal}`:'該当なし'; $('conduitWireArea').textContent=`${formatNumber(wireArea,1)}mm²`; $('conduitPipeArea').textContent=displayPipe?`${formatNumber(displayPipe.pipeArea,1)}mm²`:'-';
  $('conduitRecommendationReason').textContent=minimum?(reasons.length?`占有率基準を満たす最小配管 ${minimum.nominal} を推奨します。ただし、${reasons.join('、')}ため、施工性を考慮して1サイズ上も検討してください。外径・内径は参考値です。`:`占有率基準を満たす最小配管 ${minimum.nominal} を推奨します。外径・内径は参考値です。`):'選択した電線管種類の最大サイズでも設定占有率を満足しません。配管種類、分割配管または施工条件を見直してください。';
}
...
function buildDataIssuesDoc(){ return {note:'寸法・質量データの収録状況です。CD管42、PF管42、VE管質量は参考値として反映済みです。最終値は採用品の仕様書で確認してください。', headers:['No','内容'], keys:['no','text'], cardLayout:true, rows:DATA_ISSUES.map((text,index)=>({no:index+1,text}))}; }

function supportDiagramHtml(type){
  const common = `
    <div class="doc-diagram-card">
      <div class="doc-diagram-title">水平配管・ボックス付近</div>
      <svg class="doc-diagram-svg" viewBox="0 0 360 160" role="img" aria-label="水平配管支持イメージ">
        <rect x="22" y="62" width="316" height="26" rx="13" class="pipe-line"/>
        <rect x="268" y="45" width="54" height="60" rx="8" class="device-box"/>
        <line x1="85" y1="30" x2="85" y2="64" class="support-line"/>
        <line x1="180" y1="30" x2="180" y2="64" class="support-line"/>
        <line x1="250" y1="30" x2="250" y2="64" class="support-line accent"/>
        <path d="M85 112 H180" class="dimension-line"/>
        <text x="132" y="135" text-anchor="middle">標準支持間隔</text>
        <text x="250" y="24" text-anchor="middle">端部付近支持</text>
      </svg>
    </div>`;
  const vertical = `
    <div class="doc-diagram-card">
      <div class="doc-diagram-title">垂直配管・立上り</div>
      <svg class="doc-diagram-svg" viewBox="0 0 360 160" role="img" aria-label="垂直配管支持イメージ">
        <rect x="170" y="18" width="28" height="124" rx="14" class="pipe-line"/>
        <line x1="198" y1="40" x2="285" y2="40" class="support-line"/>
        <line x1="198" y1="82" x2="285" y2="82" class="support-line"/>
        <line x1="198" y1="124" x2="285" y2="124" class="support-line"/>
        <rect x="282" y="20" width="16" height="124" class="wall-line"/>
        <path d="M128 40 V82" class="dimension-line"/>
        <path d="M128 82 V124" class="dimension-line"/>
        <text x="92" y="86" text-anchor="middle">一般支持</text>
        <text x="180" y="154" text-anchor="middle">自重・各階支持も確認</text>
      </svg>
    </div>`;
  const resin = `
    <div class="doc-diagram-card">
      <div class="doc-diagram-title">樹脂管のたわみ・熱伸縮</div>
      <svg class="doc-diagram-svg" viewBox="0 0 360 160" role="img" aria-label="合成樹脂管支持イメージ">
        <path d="M35 76 C95 46 130 106 188 76 S280 46 330 76" class="pipe-stroke"/>
        <line x1="70" y1="30" x2="70" y2="64" class="support-line"/>
        <line x1="180" y1="30" x2="180" y2="64" class="support-line"/>
        <line x1="290" y1="30" x2="290" y2="64" class="support-line"/>
        <path d="M80 120 H280" class="dimension-line"/>
        <text x="180" y="144" text-anchor="middle">支持間隔を短めに確認</text>
      </svg>
    </div>`;
  const flexible = `
    <div class="doc-diagram-card">
      <div class="doc-diagram-title">可とう管・機器接続部</div>
      <svg class="doc-diagram-svg" viewBox="0 0 360 160" role="img" aria-label="可とう管支持イメージ">
        <rect x="255" y="48" width="70" height="70" rx="10" class="device-box"/>
        <path d="M35 84 C95 38 150 130 220 84 L255 84" class="flex-line"/>
        <line x1="55" y1="34" x2="55" y2="70" class="support-line"/>
        <line x1="228" y1="36" x2="228" y2="80" class="support-line accent"/>
        <text x="228" y="25" text-anchor="middle">接続部付近</text>
        <text x="155" y="145" text-anchor="middle">曲げ半径・余長・緩み止め</text>
      </svg>
    </div>`;
  const rack = `
    <div class="doc-diagram-card">
      <div class="doc-diagram-title">ラック支持・支持元強度</div>
      <svg class="doc-diagram-svg" viewBox="0 0 360 160" role="img" aria-label="ラック支持イメージ">
        <rect x="42" y="92" width="276" height="18" rx="4" class="rack-line"/>
        <line x1="82" y1="20" x2="82" y2="92" class="hanger-line"/>
        <line x1="180" y1="20" x2="180" y2="92" class="hanger-line"/>
        <line x1="278" y1="20" x2="278" y2="92" class="hanger-line"/>
        <rect x="35" y="12" width="290" height="10" class="ceiling-line"/>
        <circle cx="120" cy="82" r="6" class="cable-dot"/><circle cx="142" cy="82" r="6" class="cable-dot"/><circle cx="164" cy="82" r="6" class="cable-dot"/>
        <path d="M70 132 H290" class="dimension-line"/>
        <text x="180" y="154" text-anchor="middle">ラック自重＋ケーブル質量＋地震時荷重</text>
      </svg>
    </div>`;
  const seismic = `
    <div class="doc-diagram-card">
      <div class="doc-diagram-title">耐震支持・振れ止め</div>
      <svg class="doc-diagram-svg" viewBox="0 0 360 160" role="img" aria-label="耐震支持イメージ">
        <rect x="52" y="95" width="256" height="18" rx="4" class="rack-line"/>
        <line x1="92" y1="20" x2="92" y2="95" class="hanger-line"/>
        <line x1="268" y1="20" x2="268" y2="95" class="hanger-line"/>
        <line x1="92" y1="20" x2="160" y2="95" class="brace-line"/>
        <line x1="268" y1="20" x2="200" y2="95" class="brace-line"/>
        <rect x="44" y="12" width="272" height="10" class="ceiling-line"/>
        <path d="M160 122 C170 132 190 132 200 122" class="shake-line"/>
        <text x="180" y="151" text-anchor="middle">斜材・アンカー・母材強度を確認</text>
      </svg>
    </div>`;
  const map = {conduit: common + vertical + resin + flexible, rack: rack, seismic: seismic};
  return `<div class="doc-diagram-grid">${map[type] || common}</div>`;
}

function buildConduitSupportDoc(){
  return {
    note:'配管支持間隔の参考表です。一般支持間隔と、垂直配管・端部・曲がり部などの追加確認を分けて表示します。最終判断は設計図書、公共建築工事標準仕様書、標準施工要領、内線規程、採用品仕様書を優先してください。',
    diagramHtml:supportDiagramHtml('conduit'),
    headers:['対象','一般支持間隔','垂直方向・立上り','端部・曲がり部の注意','根拠・確認資料','主な確認事項'],
    keys:['target','interval','vertical','edge','basis','note'],
    cardLayout:true,
    rows:[
      {target:'金属管（厚鋼・薄鋼・ねじなし鋼製電線管）',interval:'2.0m以下',vertical:'一般支持間隔2.0m以下を基本に、自重支持・各階支持・耐震支持を別途確認',edge:'ボックス、器具、曲がり部、分岐部、管端付近は必要に応じ支持',basis:'公共建築工事標準仕様書、標準施工要領、内線規程、設計図書',note:'屋外、振動部、重量物支持部、盤入線部は設計図・仕様書を優先'},
      {target:'合成樹脂管（CD管・PF管・VE管等）',interval:'1.5m以下を目安',vertical:'たわみ・抜け・熱伸縮を考慮し、立上り部・各階・貫通部で固定を確認',edge:'端部、曲がり部、ボックス付近はたわみ・抜けを確認',basis:'標準施工要領、内線規程、採用品仕様書、設計図書',note:'熱伸縮、屋外紫外線、埋設/露出条件を確認'},
      {target:'金属製可とう電線管',interval:'1.0m以下を目安',vertical:'垂直・機器接続部は短い間隔で支持し、余長・振動吸収を確認',edge:'接続部付近・機器振動部は緩み止め、曲げ半径を確認',basis:'標準施工要領、採用品仕様書、設計図書',note:'防水形/非防水形、振動機器、可動部の余長を確認'},
      {target:'FEP管・地中管路',interval:'埋設条件・管路設計による',vertical:'立上り部、ハンドホール取合い、管口部を固定し防護を確認',edge:'ハンドホール、立上り、曲がり部、管口処理を確認',basis:'設計図書、施工要領、採用品仕様書',note:'土被り、埋戻し、曲げ、管路勾配、通線張力を別途確認'},
      {target:'共通',interval:'支持間隔だけでなく支持強度で確認',vertical:'垂直荷重、ずれ止め、貫通部、盤入線部、地震時変位を確認',edge:'管端・盤入線部・貫通部・プルボックス前後を確認',basis:'設計図書、仕様書、施工要領、関係規程',note:'数値は参考。支持元・アンカー・母材強度を確認'}
    ]
  };
}
...
function buildRackSeismicDoc(){
  return {
    note:'ラック耐震の考え方です。耐震種別は設計図書・仕様書・耐震計算書を優先し、重要度、設置階、支持方法、支持元強度で確認します。',
    diagramHtml:supportDiagramHtml('seismic'),
    headers:['項目','内容','主な基準・確認'],
    keys:['title','body','basis'],
    cardLayout:true,
    rows:[
      {title:'耐震種別',body:'SA種、A種、B種などの指定を確認。案件仕様により表記・適用範囲が異なるため、設計図書を優先',basis:'公共建築工事標準仕様書、公共建築設備工事標準図、建築設備耐震設計・施工指針、設計図書'},
      {title:'SA種相当',body:'重要設備・高い耐震性能を求める箇所で指定される場合がある。水平・鉛直地震力、斜材、アンカー、母材強度を確認',basis:'設計図書・耐震計算書で指定値を確認'},
      {title:'A種相当',body:'一般より強い耐震支持を求める箇所で指定される場合がある。横引き、立上り、機器接続部の振れ止めを確認',basis:'標準図、設計図書、重要度・設置階条件を確認'},
      {title:'B種相当',body:'比較的一般的な耐震支持として指定される場合がある。自重支持に加えて斜材・振れ止め・アンカー条件を確認',basis:'標準図、設計図書、施工要領を確認'},
      {title:'支持元強度',body:'ラック自重、ケーブル質量、付属品、将来増設、地震時荷重を見込み、天井・壁・梁・床・架台・アンカーの強度を確認',basis:'構造体条件、アンカー仕様、母材強度、耐震計算書'},
      {title:'施工メモ',body:'長尺ラック、重量ケーブル、屋上、機械室、立上り、伸縮部、振動部では振れ止め・変位吸収・支持点を重点確認',basis:'設計図書・仕様書を優先'}
    ]
  };
}
...
function buildTechCalcToolsDoc(){
  return {
    note:'任意の数値を代入して概算検算できます。計算結果本体とは分離した補助計算です。入力欄は空欄から開始し、必要な値だけ入力してください。',
    ...
    html:`
      <div class="tech-calc-list optional-calc-list">
        <section class="tech-calc-card optional-calc-card" data-tech-calc="three-phase-current">
          <div class="tech-calc-head"><h4>三相電流計算</h4><span class="mode-badge none">任意入力</span></div>
          <p class="field-hint">容量・電圧・力率・効率を入力して、三相負荷電流を概算します。</p>
          <div class="optional-calc-input-title">入力値</div>
          <div class="tech-calc-grid">
            <label><span>容量 [kW]</span><input data-field="kw" type="number" inputmode="decimal" placeholder="例：45"></label>
            <label><span>電圧 [V]</span><input data-field="voltage" type="number" inputmode="decimal" placeholder="例：440"></label>
            <label><span>力率</span><input data-field="pf" type="number" inputmode="decimal" placeholder="例：0.85" step="0.01"></label>
            <label><span>効率</span><input data-field="eff" type="number" inputmode="decimal" placeholder="例：0.90" step="0.01"></label>
          </div>
          <div class="optional-calc-actions"><button type="button" class="primary small" data-calc-action="calc">計算する</button><button type="button" class="ghost small" data-calc-action="clear">クリア</button></div>
          <div class="optional-calc-result"><span>計算結果</span><strong data-result>未計算</strong></div>
        </section>
        <section class="tech-calc-card optional-calc-card" data-tech-calc="single-phase-current">
          <div class="tech-calc-head"><h4>単相電流計算</h4><span class="mode-badge none">任意入力</span></div>
          <p class="field-hint">容量・電圧・力率・効率を入力して、単相負荷電流を概算します。</p>
          <div class="optional-calc-input-title">入力値</div>
          <div class="tech-calc-grid">
            <label><span>容量 [kW]</span><input data-field="kw" type="number" inputmode="decimal" placeholder="例：5"></label>
            <label><span>電圧 [V]</span><input data-field="voltage" type="number" inputmode="decimal" placeholder="例：200"></label>
            <label><span>力率</span><input data-field="pf" type="number" inputmode="decimal" placeholder="例：0.90" step="0.01"></label>
            <label><span>効率</span><input data-field="eff" type="number" inputmode="decimal" placeholder="例：1.00" step="0.01"></label>
          </div>
          <div class="optional-calc-actions"><button type="button" class="primary small" data-calc-action="calc">計算する</button><button type="button" class="ghost small" data-calc-action="clear">クリア</button></div>
          <div class="optional-calc-result"><span>計算結果</span><strong data-result>未計算</strong></div>
        </section>
        <section class="tech-calc-card optional-calc-card" data-tech-calc="voltage-drop">
          <div class="tech-calc-head"><h4>電圧降下計算</h4><span class="mode-badge none">任意入力</span></div>
          <p class="field-hint">電流・導体抵抗・長さ・電圧を入力して、電圧降下と率を概算します。</p>
          <div class="optional-calc-input-title">入力値</div>
          <div class="tech-calc-grid">
            <label><span>方式</span><select data-field="phase"><option value="three">三相3線</option><option value="single2">単相2線</option></select></label>
            <label><span>電流 [A]</span><input data-field="current" type="number" inputmode="decimal" placeholder="例：225"></label>
            <label><span>導体抵抗 [Ω/km]</span><input data-field="resistance" type="number" inputmode="decimal" placeholder="例：0.187" step="0.001"></label>
            <label><span>長さ [m]</span><input data-field="length" type="number" inputmode="decimal" placeholder="例：120"></label>
            <label><span>電圧 [V]</span><input data-field="voltage" type="number" inputmode="decimal" placeholder="例：440"></label>
          </div>
          <div class="optional-calc-actions"><button type="button" class="primary small" data-calc-action="calc">計算する</button><button type="button" class="ghost small" data-calc-action="clear">クリア</button></div>
          <div class="optional-calc-result"><span>計算結果</span><strong data-result>未計算</strong></div>
        </section>
        <section class="tech-calc-card optional-calc-card" data-tech-calc="ampacity-correction">
          <div class="tech-calc-head"><h4>許容電流補正</h4><span class="mode-badge none">任意入力</span></div>
          <p class="field-hint">基準許容電流と各補正係数を入力して、補正後許容電流を概算します。</p>
          <div class="optional-calc-input-title">入力値</div>
          <div class="tech-calc-grid">
            <label><span>基準許容電流 [A]</span><input data-field="base" type="number" inputmode="decimal" placeholder="例：290"></label>
            <label><span>温度補正</span><input data-field="temp" type="number" inputmode="decimal" placeholder="例：1.00" step="0.01"></label>
            <label><span>条数補正</span><input data-field="parallel" type="number" inputmode="decimal" placeholder="例：1.00" step="0.01"></label>
            <label><span>敷設方法補正</span><input data-field="method" type="number" inputmode="decimal" placeholder="例：0.95" step="0.01"></label>
            <label><span>敷設条件補正</span><input data-field="condition" type="number" inputmode="decimal" placeholder="例：1.00" step="0.01"></label>
          </div>
          <div class="optional-calc-actions"><button type="button" class="primary small" data-calc-action="calc">計算する</button><button type="button" class="ghost small" data-calc-action="clear">クリア</button></div>
          <div class="optional-calc-result"><span>計算結果</span><strong data-result>未計算</strong></div>
        </section>
...
function updateTechCalcCard(card){
  ...
  if (type === 'three-phase-current') {
    ...
    out.textContent = kw && v && pf && eff ? `${formatNumber(kw*1000/(Math.sqrt(3)*v*pf*eff),2)}A` : '未計算';
  } else if (type === 'single-phase-current') {
    const kw = techCalcNumber(card,'kw'), v = techCalcNumber(card,'voltage'), pf = techCalcNumber(card,'pf'), eff = techCalcNumber(card,'eff');
    out.textContent = kw && v && pf && eff ? `${formatNumber(kw*1000/(v*pf*eff),2)}A` : '未計算';
    ...
    out.textContent = i && r && l && v ? `${formatNumber(dropV,2)}V / ${formatNumber(dropV/v*100,2)}%` : '未計算';
    ...
    out.textContent = base ? `${formatNumber(corrected,2)}A` : '未計算';
  }
}
function bindTechCalcTools(){
  document.querySelectorAll('[data-tech-calc]').forEach(card=>{
    ...
    card.querySelector('[data-calc-action="calc"]')?.addEventListener('click',()=>updateTechCalcCard(card));
    card.querySelector('[data-calc-action="clear"]')?.addEventListener('click',()=>{
      card.querySelectorAll('input').forEach(el=>{ el.value = ''; });
      const out = card.querySelector('[data-result]');
      if (out) out.textContent = '未計算';
    });
...
let pendingDocItemKey = null;
function openDocsToItem(itemKey){ pendingDocItemKey = itemKey; switchScreen('docs'); renderDocs(); }
function buildDocDataCards(rows = [], headers = [], keys = []){
  const root = document.createElement('div');
  root.className = 'doc-data-list';
  if (!rows.length) {
    root.innerHTML = '<div class="doc-data-card muted">表示するデータはありません。</div>';
    return root;
  }
  rows.forEach(row=>{
    const card = document.createElement('div');
    card.className = 'doc-data-card';
    keys.forEach((key, cellIndex)=>{
      const line = document.createElement('div');
      line.className = cellIndex === 0 ? 'doc-data-line primary-line' : 'doc-data-line';
      const label = document.createElement('span');
      label.textContent = headers[cellIndex] || key;
...
function renderDocs(){
  ...
  if (pendingDocItemKey) {
    const pendingIndex = flatItems.findIndex(entry=>entry.itemKey === pendingDocItemKey);
    pendingDocItemKey = null;
    if (pendingIndex >= 0) {
      const pending = flatItems[pendingIndex];
      expandedDocGroups.add(pending.group.key);
      expandedDocItems.clear();
      expandedDocItems.add(pending.itemKey);
    }
  }
  visibleGroups.forEach(({group, visibleItems})=>{
    if (!visibleItems.length && search) return;
    const groupCard = document.createElement('div'); groupCard.className = `doc-group-card doc-group-${group.key}`;
    ...
    visibleItems.forEach(entry=>{
      ...
      const itemCard = document.createElement('div'); itemCard.className = 'doc-item-card doc-tree-item'; itemCard.id = `doc-card-${safeKey}`;
      ...
      if (payload.diagramHtml) { const dbox = document.createElement('div'); dbox.className = 'doc-diagram-html'; dbox.innerHTML = payload.diagramHtml; itemBody.appendChild(dbox); }
      ...
      if (payload.html) {
        ...
        payload.sections.forEach((section, sectionIndex)=>{
          ...
          sectionBody.appendChild(buildDocDataCards(section.rows || [], sectionHeaders, sectionKeys));
        ...
        itemBody.appendChild(buildDocDataCards(payload.rows || [], tableHeaders, keys));
      }
...
function bindEvents(){
  ...
  $('docsSearch').addEventListener('input', renderDocs); $('openCalcToolsFromCalc')?.addEventListener('click',()=>openDocsToItem('calc_tools:tech_calc_tools')); $('openDocsFromCalc').addEventListener('click',()=>switchScreen('docs')); $('returnCalcFromDocs').addEventListener('click',()=>switchScreen('calc')); setTextIfExists('openCalcToolsFromCalc','任意計算'); setTextIfExists('openDocsFromCalc','技術資料');
