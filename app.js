...
const VERSIONS = { app:'Web v3.0.0', ampacity:'2026.04-A', physical:'2026.04-B', form:'2026.04-C' };
const STORAGE = {
...
const LONG_TEXT_FIELDS = new Set(['否理由','選定根拠']);
...
const CABLE_DATA = {
  ...
  'CVT': {
    8:{outerDiameter:15.5,massKgKm:280,resistance:2.36,ampacity:66},
    14:{outerDiameter:18.0,massKgKm:410,resistance:1.34,ampacity:88},
    22:{outerDiameter:20.5,massKgKm:610,resistance:0.849,ampacity:113},
    38:{outerDiameter:24.0,massKgKm:970,resistance:0.491,ampacity:158},
    60:{outerDiameter:28.5,massKgKm:1490,resistance:0.311,ampacity:219},
    100:{outerDiameter:35.0,massKgKm:2390,resistance:0.187,ampacity:303},
    150:{outerDiameter:40.5,massKgKm:3410,resistance:0.124,ampacity:393},
    200:{outerDiameter:46.0,massKgKm:4540,resistance:0.0933,ampacity:478},
    250:{outerDiameter:51.0,massKgKm:5550,resistance:0.0754,ampacity:549},
    325:{outerDiameter:56.5,massKgKm:7060,resistance:0.0579,ampacity:650},
    400:{outerDiameter:62.0,massKgKm:9380,resistance:0.0471,ampacity:740}
  }
};

const PLASTIC_CONDUIT = {
  'CD管': [
    {nominal:'14', outerDiameter:19.0, innerDiameter:14.0, unitMassKgM:3.2/50},
    {nominal:'16', outerDiameter:21.0, innerDiameter:16.0, unitMassKgM:3.5/50},
    {nominal:'22', outerDiameter:27.5, innerDiameter:22.0, unitMassKgM:5.2/50},
    {nominal:'28', outerDiameter:34.0, innerDiameter:28.0, unitMassKgM:4.0/30},
    {nominal:'36', outerDiameter:42.0, innerDiameter:36.0, unitMassKgM:5.7/30}
  ],
  'PF管': [
    {nominal:'14', outerDiameter:21.5, innerDiameter:14.0, unitMassKgM:3.6/50},
    {nominal:'16', outerDiameter:23.0, innerDiameter:16.0, unitMassKgM:4.2/50},
    {nominal:'22', outerDiameter:30.5, innerDiameter:22.0, unitMassKgM:6.0/50},
    {nominal:'28', outerDiameter:36.5, innerDiameter:28.0, unitMassKgM:4.8/30},
    {nominal:'36', outerDiameter:45.5, innerDiameter:36.0, unitMassKgM:6.5/30}
  ],
  'FEP管': [
    {nominal:'30', outerDiameter:'-', innerDiameter:'-', unitMassKgM:'-'},
    {nominal:'50', outerDiameter:'-', innerDiameter:'-', unitMassKgM:'-'},
    {nominal:'65', outerDiameter:'-', innerDiameter:'-', unitMassKgM:'-'}
  ],
  'VE管': [
    {nominal:'16', outerDiameter:'-', innerDiameter:'-', unitMassKgM:'-'},
    {nominal:'22', outerDiameter:'-', innerDiameter:'-', unitMassKgM:'-'},
    {nominal:'28', outerDiameter:'-', innerDiameter:'-', unitMassKgM:'-'},
    {nominal:'36', outerDiameter:'-', innerDiameter:'-', unitMassKgM:'-'}
  ]
};

const METAL_CONDUIT = {
  '鋼製電線管': [
    {nominal:'G16', outerDiameter:21.0, innerDiameter:(21.0-2*2.3).toFixed(1), unitMassKgM:3.88/3.66},
    {nominal:'G22', outerDiameter:26.5, innerDiameter:(26.5-2*2.3).toFixed(1), unitMassKgM:5.01/3.66},
    {nominal:'G28', outerDiameter:33.3, innerDiameter:(33.3-2*2.5).toFixed(1), unitMassKgM:6.95/3.66},
    {nominal:'G36', outerDiameter:41.9, innerDiameter:(41.9-2*2.5).toFixed(1), unitMassKgM:8.89/3.66},
    {nominal:'G42', outerDiameter:47.8, innerDiameter:(47.8-2*2.5).toFixed(1), unitMassKgM:10.2/3.66},
    {nominal:'G54', outerDiameter:59.6, innerDiameter:(59.6-2*2.8).toFixed(1), unitMassKgM:14.3/3.66}
  ],
  'ねじなし鋼製電線管': [
    {nominal:'E19', outerDiameter:19.1, innerDiameter:(19.1-2*1.2).toFixed(1), unitMassKgM:1.94/3.66},
    {nominal:'E25', outerDiameter:25.4, innerDiameter:(25.4-2*1.2).toFixed(1), unitMassKgM:2.62/3.66},
    {nominal:'E31', outerDiameter:31.8, innerDiameter:(31.8-2*1.4).toFixed(1), unitMassKgM:3.84/3.66},
    {nominal:'E39', outerDiameter:38.1, innerDiameter:(38.1-2*1.4).toFixed(1), unitMassKgM:4.65/3.66},
    {nominal:'E51', outerDiameter:50.8, innerDiameter:(50.8-2*1.4).toFixed(1), unitMassKgM:6.26/3.66},
    {nominal:'E63', outerDiameter:63.5, innerDiameter:(63.5-2*1.6).toFixed(1), unitMassKgM:8.93/3.66}
  ],
  '薄鋼電線管': [
    {nominal:'C19', outerDiameter:19.1, innerDiameter:(19.1-2*1.6).toFixed(1), unitMassKgM:2.53/3.66},
    {nominal:'C25', outerDiameter:25.4, innerDiameter:(25.4-2*1.6).toFixed(1), unitMassKgM:3.44/3.66},
    {nominal:'C31', outerDiameter:31.8, innerDiameter:(31.8-2*1.6).toFixed(1), unitMassKgM:4.36/3.66},
    {nominal:'C39', outerDiameter:38.1, innerDiameter:(38.1-2*1.6).toFixed(1), unitMassKgM:5.27/3.66},
    {nominal:'C51', outerDiameter:50.8, innerDiameter:(50.8-2*1.6).toFixed(1), unitMassKgM:7.1/3.66},
    {nominal:'C63', outerDiameter:63.5, innerDiameter:(63.5-2*2.0).toFixed(1), unitMassKgM:11.1/3.66}
  ],
  'プリカチューブ（防水）': [
    {nominal:'16', outerDiameter:'-', innerDiameter:'-', unitMassKgM:'-'},
    {nominal:'24', outerDiameter:'-', innerDiameter:'-', unitMassKgM:'-'},
    {nominal:'30', outerDiameter:'-', innerDiameter:'-', unitMassKgM:'-'}
  ],
  'プリカチューブ（非防水）': [
    {nominal:'17', outerDiameter:'-', innerDiameter:'-', unitMassKgM:'-'},
    {nominal:'24', outerDiameter:'-', innerDiameter:'-', unitMassKgM:'-'},
    {nominal:'30', outerDiameter:'-', innerDiameter:'-', unitMassKgM:'-'}
  ],
  'ポリエチレンライニング鋼管': [
...
const DEFAULT_UI = {
  ...
  wiringLength:'', existingBreaker:'', projectName:'', projectRemarks:'', loadCount:'',
  cableFamily:'', cableSubtype:'', cableSize:'', baseAmpacity:'', ampacityMode:'none',
  installationMethod:'', layingCondition:'', ambientTemperature:'40', parallelCount:'',
  loads:[], lastResult:null, compareBaseId:'', compareMode:'major', readOnly:false
};
...
let currentDocPayload = null;
let confirmAction = null;
let openDocGroupKey = null;
...
function getCableSizes(type){
  if (!type || !CABLE_DATA[type]) return [];
  return Object.keys(CABLE_DATA[type]).map(Number).sort((a,b)=>a-b);
}

function getCableInfo(type, size){
  if (!type || !size || !CABLE_DATA[type]) return null;
  return CABLE_DATA[type][size] || null;
}

function createBlankLoad(index){
  return {id:crypto.randomUUID(), title:`負荷${index+1}`, name:'', inputType:'', value:''};
}
...
function sizingCurrentBase(){
  return state.calcMode === 'existing' ? adoptedBreaker() : requiredBreakerFromLoad();
}

function getCorrectionBreakdown(){
  const temp = Number(state.ambientTemperature || 0);
  const parallel = Number(state.parallelCount || 0);
  ...
  const methodValue =
    state.installationMethod === '気中' ? installation.air :
    state.installationMethod === 'ラック' ? installation.rack :
    state.installationMethod === '配管' ? installation.pipe : null;

  const final = [temperature, parallelCoef, methodValue, condition].every(v=>v!=null)
    ? temperature * parallelCoef * methodValue * condition
    : null;

  return {
    temperature,
    parallel:parallelCoef,
    air:installation.air,
    rack:installation.rack,
...
function correctedAmpacity(){
  const basis = Number(state.baseAmpacity || 0);
...
function rackWidthFor(outerDiameter, count){
  ...
  const width = RACK_WIDTHS.find(v=>v>=occupied) || RACK_WIDTHS.at(-1);
  return `W${width}`;
}

function suggestedCableSelection(){
  const type = currentCableType();
  const sizes = getCableSizes(type);
  if (!type || !sizes.length) return null;

  const designCurrent = sizingCurrentBase();
  const loadSummary = getLoadSummary();
  const breakdown = getCorrectionBreakdown();
  const length = Number(state.wiringLength || 0);
  if (!designCurrent || !length || !breakdown.final) return null;

  const candidates = sizes.map(size=>{
    const info = getCableInfo(type,size);
    const basis =
      (state.ampacityMode==='manual' && Number(state.cableSize)===size && Number(state.baseAmpacity))
    ...
    return {
      ...
      corrected,
      drop,
      currentOk:corrected>=designCurrent,
      dropOk:drop.dropPercent<=DROP_LIMIT_PERCENT
    };
  });
...
function selectedCableTypeFromState(){
...
function applyCableTypeToState(type){
  ...
  state.cableFamily = 'CV';

  if (type === 'CV-1C') {
    state.cableSubtype = 'CV-単芯';
    return;
  }

  state.cableSubtype = type || '';
}

function missingFields(){
  const missing = [];
  const pushIf = (condition, label, id) => {
  ...
  state.loads.forEach((load, index) => {
    ...
    if (load.value === '' || load.value === null || load.value === undefined) {
      missing.push({ label:`負荷${index + 1}の負荷値を入力してください。`, id:`load-value-${load.id}` });
    }
  });
...
function calculate(){
  ...
  const basis = candidate?.basis || Number(state.baseAmpacity || 0);
  const corrected = candidate?.corrected || correctedAmpacity();

  const drop = candidate
    ? calcVoltageDrop(loadSummary.totalCurrent, Number(candidate.info?.resistance || 0), Number(state.wiringLength || 0))
    : { dropV:0, dropPercent:0 };

  const breakerMarginPercent = adopted
    ? ((adopted - loadSummary.totalCurrent) / adopted) * 100
    : 0;

  const capacityMarginKW = adopted
    ? (
        isThreePhase()
          ? (Math.sqrt(3) * Number(state.voltage) * adopted * Number(state.powerFactor) * Number(state.efficiency) / 1000)
          : (Number(state.voltage) * adopted * Number(state.powerFactor) * Number(state.efficiency) / 1000)
      ) - loadSummary.totalKW
    : 0;
  ...
  const massKgM = candidate?.info?.massKgKm
    ? Number(candidate.info.massKgKm) / 1000
    : 0;
  ...
  const result = {
    ...
    cableType:selectedCableTypeFromState(),
    ...
    existingBreakerJudge:state.calcMode === 'existing' ? (breakerOk ? '適合' : '否') : '-',
  ...
  r.loadDetails.forEach((item, index) => {
    ...
    card.innerHTML = `
      ...
      <div class="readonly-grid top-gap">
        <div><span>入力方式</span><strong>${escapeHtml(item.inputType || '-')}</strong></div>
        <div><span>負荷値</span><strong>${escapeHtml(item.value || '-')} ${escapeHtml(item.inputType || '')}</strong></div>
        <div><span>換算電流 [A]</span><strong>${formatNumber(item.current, 2)}</strong></div>
        <div><span>換算容量 [kW]</span><strong>${formatNumber(item.kw, 3)}</strong></div>
        <div><span>換算容量 [kVA]</span><strong>${formatNumber(item.kva, 3)}</strong></div>
      </div>
    `;
  ...
  state.loads.forEach((load, index) => {
    ...
    card.innerHTML = `
      ...
      <div class="readonly-grid top-gap">
        ...
        <div><span>換算容量 [kW]</span><strong id="load-kw-${load.id}">${formatNumber(kwForLoad(load), 3)}</strong></div>
        <div><span>換算容量 [kVA]</span><strong id="load-kva-${load.id}">${formatNumber(kvaForLoad(load), 3)}</strong></div>
      </div>
    `;
...
function updateLoadDerivedValues(load){
  const typeLabel = document.getElementById(`load-type-label-${load.id}`);
  const currentEl = document.getElementById(`load-current-${load.id}`);
  const kwEl = document.getElementById(`load-kw-${load.id}`);
  const kvaEl = document.getElementById(`load-kva-${load.id}`);

  if (typeLabel) typeLabel.textContent = load.inputType || '-';
  if (currentEl) currentEl.textContent = formatNumber(currentForLoad(load), 2);
  if (kwEl) kwEl.textContent = formatNumber(kwForLoad(load), 3);
  if (kvaEl) kvaEl.textContent = formatNumber(kvaForLoad(load), 3);
}
...
function buildCableTable(type, label){
  const rows = Object.entries(CABLE_DATA[type] || {}).map(([size, info]) => ({
    sizeSq:size,
  ...
  return {
    ...
    headers:['サイズ [sq]','外径 [mm]','概算質量 [kg/km]','導体抵抗 [Ω/km]','許容電流 [A]'],
    keys:['sizeSq','outerDiameter','massKgKm','resistance','ampacity'],
    rows
  };
}
...
function buildVoltageCriteria(){
  return textDoc(
    '電圧降下の判定基準はアプリ内で5.0%以下に設定しています。',
    [
      ['判定基準', `${DROP_LIMIT_PERCENT.toFixed(1)}%以下`],
      ['注意', '長距離配線・始動電流が大きい負荷・電圧変動が厳しい設備では、仕様条件を優先してください。'],
      ['実務メモ', 'アプリは参考計算です。最終判断は現場条件・内線規定・設計仕様で確認してください。']
    ]
  );
}

function buildFormulaCurrent(){
  return textDoc(
    '電流換算の基本式です。',
    [
      ['三相 kW → A', 'I = P × 1000 ÷ (√3 × V × 力率 × 効率)'],
      ['単相 kW → A', 'I = P × 1000 ÷ (V × 力率 × 効率)'],
      ['三相 kVA → A', 'I = kVA × 1000 ÷ (√3 × V)'],
      ['単相 kVA → A', 'I = kVA × 1000 ÷ V']
    ]
  );
}

function buildFormulaDrop(){
  return textDoc(
    '電圧降下の基本式です。',
    [
      ['三相', 'e = √3 × I × R × L ÷ 1000'],
      ['単相', 'e = 2 × I × R × L ÷ 1000'],
      ['百分率', '電圧降下率[%] = e ÷ 電圧 × 100'],
      ['注意', 'リアクタンス成分は簡易計算では省略しています。長距離・大容量では詳細検討してください。']
    ]
  );
}

function buildFormulaCoef(){
  return textDoc(
    '許容電流補正の基本式です。',
    [
      ['補正後許容電流', '基準許容電流 × 温度補正 × 条数補正 × 敷設方法補正 × 敷設条件補正'],
      ['温度補正', '20〜50℃の係数テーブルを使用'],
      ['条数補正', '1〜6条の係数テーブルを使用'],
      ['注意', '密集・日射・管路・ラック等の現場条件を反映して確認してください。']
    ]
  );
}

function buildMainBreakerDoc(){
  return textDoc(
    '主幹選定の考え方です。',
    [
      ['自動選定', '合計電流以上となる最小の標準開閉器を選定します。'],
      ['既設開閉器指定', '既設開閉器容量を基準に、幹線許容電流・電圧降下を判定します。'],
      ['裕度', '開閉器裕度[%] = (採用開閉器 - 合計電流) ÷ 採用開閉器 × 100'],
      ['注意', '始動電流・需要率・保護協調・短絡容量は別途確認してください。']
    ]
  );
}
...
function buildConduitRows(dataset, groupName){
  ...
  Object.entries(dataset || {}).forEach(([type, items]) => {
    items.forEach(info => {
      rows.push({
        group:groupName,
        conduitType:type,
        ...
        length:info.lengthM ? `${info.lengthM}m` : info.lengthMm ? `${info.lengthMm}mm` : info.coilLengthM ? `${info.coilLengthM}m/巻` : '-',
        coilMassKg:info.coilMassKg ?? '-',
        minBendRadiusMm:info.minBendRadiusMm ?? '-',
        note:info.note || (info.coatingThicknessMm ? `膜厚 ${info.coatingThicknessMm}mm` : '')
      });
    });
  });
...
function buildConduitPayload(dataset, groupName, note){
  return {
    note,
    headers:['分類','配管種類','呼び','外径 [mm]','内径/近似内径 [mm]','厚さ [mm]','単位質量 [kg/m]','長さ','巻質量 [kg]','最小曲げ半径 [mm]','備考'],
    keys:['group','conduitType','nominal','outerDiameter','innerDiameter','thicknessMm','unitMassKgM','length','coilMassKg','minBendRadiusMm','note'],
    rows:buildConduitRows(dataset, groupName)
  };
}

function buildResinConduitDoc(){
  return buildConduitPayload(
    PLASTIC_CONDUIT,
    '樹脂系',
    'CD管・PF管・FEP管・VE管の参考寸法表です。アプリ内ではメーカー名・商品名を表示しません。'
  );
}
...
function buildFlexibleConduitDoc(){
  const dataset = typeof FLEXIBLE_CONDUIT === 'undefined' ? {} : FLEXIBLE_CONDUIT;
  return buildConduitPayload(
    dataset,
    '可とう管',
...
function buildDataIssuesDoc(){
  const issues = typeof DATA_ISSUES === 'undefined' ? ['未確定データ一覧は未定義です。'] : DATA_ISSUES;
  return {
    note:'現時点で不足または要確認として残しているデータです。',
    headers:['No','内容'],
    keys:['no','text'],
    rows:issues.map((text, index) => ({ no:index + 1, text }))
  };
}
...
function buildConduitSupportDoc(){
  return textDoc(
    '配管支持間隔の参考メモです。',
    [
      ['確認事項', '配管種類、サイズ、支持材、屋内外、振動、地震時条件を確認してください。'],
      ['注意', 'メーカー仕様・施工要領・現場仕様を優先してください。'],
      ['アプリでの扱い', '本表は参考メモです。支持間隔そのものの自動判定は未実装です。']
    ]
  );
}
...
function buildRackWidthDoc(){
  return textDoc(
    'ラック幅選定の参考メモです。',
    [
      ['概算式', '外径 × 条数 × 1.35 を占有幅目安として標準ラック幅へ丸めます。'],
      ['注意', '多段積み、離隔、放熱、将来増設、支持条件は別途確認してください。'],
      ['アプリでの扱い', '参考ラック幅はケーブル外径と条数から概算表示します。']
    ]
  );
}
...
function buildRackSupportDoc(){
  return textDoc(
    'ラック支持間隔の参考メモです。',
    [
      ['確認事項', 'ラック幅、積載質量、支持材、アンカー、吊りボルト、地震時条件を確認してください。'],
      ['注意', '最終判断は製品仕様・設計図・構造条件で確認してください。']
    ]
  );
}

function buildRackSeismicDoc(){
  return textDoc(
    'ラック耐震の考え方です。',
    [
      ['確認事項', '重要度、設置階、支持方法、振れ止め、アンカー、天井・壁・床の強度を確認してください。'],
      ['注意', '耐震計算が必要な案件では、建築設備耐震設計指針等に基づく検討を行ってください。']
    ]
  );
}
...
function buildGroundBasisDoc(){
  return {
    ...
    rows:[
      {type:'A種', use:'高圧・特別高圧機器外箱等', resistance:'10Ω以下', note:'条件により詳細確認'},
      {type:'B種', use:'変圧器低圧側中性点等', resistance:'計算値以下', note:'地絡電流条件で算定'},
      {type:'C種', use:'300V超の低圧機器外箱等', resistance:'10Ω以下', note:'ELB条件で緩和あり'},
      {type:'D種', use:'300V以下の低圧機器外箱等', resistance:'100Ω以下', note:'ELB条件で緩和あり'}
    ]
  };
}
...
function buildHvSymbolsDoc(){
  return {
    ...
    rows:[
      ...
      {symbol:'VT', name:'計器用変圧器', body:'電圧計測・保護継電器入力に用いる。'},
      {symbol:'LA', name:'避雷器', body:'雷サージ等から設備を保護する。'}
    ]
  };
}

function ensureDocsUpgraded(){
  const construction = DOC_GROUPS.find(g => g.key === 'construction');
  if (construction) {
    if (!construction.items.some(item => item.id === 'conduit_flexible')) {
      construction.items.splice(2, 0, { id:'conduit_flexible', title:'可とう管サイズ一覧表', searchable:true, build:buildFlexibleConduitDoc });
    }
    if (!construction.items.some(item => item.id === 'data_issues')) {
      construction.items.push({ id:'data_issues', title:'未確定・要確認データ', searchable:true, build:buildDataIssuesDoc });
    }
  }
}
...
function renderDocs(){
  ...
  DOC_GROUPS.forEach(group => {
    ...
    groupCard.className = 'doc-group-card';

    const groupTitle = document.createElement('button');
    groupTitle.type = 'button';
    groupTitle.className = 'doc-group-toggle';
    groupTitle.textContent = group.title;
    groupCard.appendChild(groupTitle);

    const groupBody = document.createElement('div');
    groupBody.className = 'stack top-gap';
    ...
    group.items.forEach(item => {
      ...
      itemCard.className = 'doc-item-card';

      const itemButton = document.createElement('button');
      itemButton.type = 'button';
      itemButton.className = 'doc-item-toggle';
      itemButton.textContent = item.title;

      const itemBody = document.createElement('div');
      itemBody.className = 'top-gap';
      ...
      const rows = payload.rows.map(row => {
        const tds = keys.map(key => `<td>${escapeHtml(row[key] ?? '-')}</td>`).join('');
        return `<tr>${tds}</tr>`;
      }).join('');
      ...
      groupBody.appendChild(itemCard);
    });
  ...
  if (!root.children.length) {
    root.innerHTML = '<div class="panel muted">該当する資料はありません。</div>';
  }
}
...
function buildSaveCard(item, isHistory){
  ...
  card.innerHTML = `
    <div class="saved-line">
      <div class="saved-summary">
        <div class="saved-title-row">
          ...
          <span class="badge ${item.result?.judgement === '良' ? 'good' : 'bad'}">${escapeHtml(item.result?.judgement || '-')}</span>
        </div>
    ...
    <div class="readonly-grid top-gap">
      ...
      <div><span>採用ケーブル</span><strong>${escapeHtml(item.result?.cableType || '-')} ${item.result?.cableSize ? `${item.result.cableSize}sq` : '-'}</strong></div>
      ...
      <div><span>外径・質量データ版</span><strong>${escapeHtml(item.versions?.physical || VERSIONS.physical)}</strong></div>
    </div>

    <div class="inline-actions top-gap">
      <button class="ghost small js-open">この案を開く</button>
      ${isHistory ? '<button class="ghost small js-save-history">保存済みに追加</button>' : `<button class="ghost small js-compare">${selected ? '比較から外す' : '比較に追加'}</button>`}
  ...
  if (isHistory) {
    card.querySelector('.js-save-history').onclick = () => {
      const copied = structuredClone(item);
      copied.id = crypto.randomUUID();
      copied.title = `${item.title || '履歴'}_保存`;
      copied.savedAt = new Date().toISOString();
      savedItems.unshift(copied);
      persistState();
      renderSaved();
      showToast('保存済みに追加しました。');
    };
  } else {
...
function renderCompare(){
  ...
  const metrics = [
    ['採用ケーブル', baseItem.result?.cableType + ' ' + baseItem.result?.cableSize + 'sq', item => `${item.result?.cableType || '-'} ${item.result?.cableSize ? item.result.cableSize + 'sq' : '-'}`, null],
  ...
  const head = `
    <thead>
      <tr>
        <th>項目</th>
        <th>基準案<br>${escapeHtml(baseItem.title || '無題')}</th>
        ${targets.map(item => `<th>${escapeHtml(item.title || '無題')}</th>`).join('')}
      </tr>
    </thead>
  `;

  const body = metrics.map(([label, base, getter, type]) => {
    const cells = targets.map(item => {
      const value = getter(item);
      if (type === 'number') {
        const diff = Number(value || 0) - Number(base || 0);
        return `<td>${formatNumber(value,2)}<br><span class="muted">差分 ${diff > 0 ? '+' : ''}${formatNumber(diff,2)}</span></td>`;
      }
    ...
    return `
      <tr>
        ...
        <td>${type === 'number' ? formatNumber(base,2) : escapeHtml(base ?? '-')}</td>
        ${cells}
      </tr>
    `;
  }).join('');
...
function renderAmpacitySection(){
  ...
  const methodCoef = state.installationMethod === '気中'
    ? breakdown.air
  ...
  $('coefMethod').textContent = methodCoef ? formatNumber(methodCoef, 3) : '-';
  ...
  const info = getCableInfo(selectedCableTypeFromState(), Number(state.cableSize));
...
function renderAll(){
  ...
  setSelectOptions($('cableType'), ['CV-1C','CV-2C','CV-3C','CV-4C','CVT'], '選択してください', cableType);
...
function confirmSave(){
  ...
  const item = {
    id: existingIndex >= 0 ? savedItems[existingIndex].id : crypto.randomUUID(),
    title,
  ...
  if (existingIndex >= 0) {
    ...
    if (savedItems.length > 10) savedItems = savedItems.slice(0, 10);
    showToast('保存しました。');
  }
...
function downloadExcel(){
  ...
  const html = `
    <html>
      ...
      <body>
        <h1>低圧幹線計算 結果表</h1>
        <p class="note">本帳票は参考資料です。内線規定・関係法令・現場仕様・機器仕様を確認し、最終判断は利用者責任で行ってください。</p>

        <h2>計算結果</h2>
        <table>
          ${summaryRows.map(row => `<tr><th>${excelSafe(row[0])}</th><td>${excelSafe(row[1])}</td></tr>`).join('')}
        </table>

        <h2>負荷一覧</h2>
        <table>
          <tr><th>No</th><th>負荷名称</th><th>入力方式</th><th>負荷値</th><th>換算電流[A]</th><th>換算容量[kW]</th><th>換算容量[kVA]</th></tr>
          ${loadRows.map(row => `<tr>${row.map(cell => `<td>${excelSafe(cell)}</td>`).join('')}</tr>`).join('')}
        </table>

        <h2>施工参考メモ</h2>
        <table>
          ${r.constructionNotes.map(note => `<tr><td>${excelSafe(note)}</td></tr>`).join('')}
        </table>

        <h2>根拠メモ</h2>
        <table>
          ${r.rootMemo.map(note => `<tr><td>${excelSafe(note)}</td></tr>`).join('')}
        </table>
      </body>
    </html>
  `;
...
function downloadCsv(){
  ...
  const rows = [
    ...
    ['敷設方法補正係数', state.installationMethod],
...
function runDisclaimer(){
  ...
  $('disclaimerNextBtn').onclick = () => {
    if (disclaimerStep === 1) {
      ...
      $('disclaimerBody').textContent = '最終判断は利用者責任で行ってください。内線規定、関係法令、現場条件、機器仕様、保護協調等を必ず確認してください。';
    } else {
