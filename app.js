...
function buildMainBreakerDoc(){ return textDoc('主幹選定の考え方です。', [['必要最小開閉器','合計電流以上となる最小の標準開閉器です。'],['推奨開閉器','機器容量から算出した合計電流÷開閉器裕度設定を基準に、標準開閉器へ丸めます。既設開閉器指定時でも推奨開閉器は既設値に置き換えません。'],['既設開閉器指定','既設開閉器容量をケーブル選定基準電流として扱い、既設開閉器に見合う選定基準ケーブルを表示します。'],['自動選定','推奨開閉器容量をケーブル選定基準電流として扱います。'],['裕度','開閉器裕度[%] = (推奨開閉器 - 合計電流) ÷ 推奨開閉器 × 100'],['注意','始動電流・需要率・保護協調・短絡容量は別途確認してください。']]); }
