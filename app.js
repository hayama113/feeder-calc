...
const METAL_CONDUIT = {
  ...
  'ポリエチレンライニング鋼管': THICK_STEEL_CONDUIT.map(v => ({
    ...v,
...
function normalizeLoads(){
  ...
  while(state.loads.length < target){
    state.loads.push(createBlankLoad(state.loads.length));
  }
...
function currentForLoad(load){
  ...
  if (load.inputType === 'kW') {
    return isThreePhase()
      ? (value * 1000) / (Math.sqrt(3) * voltage * pf * eff)
      : (value * 1000) / (voltage * pf * eff);
  }
...
function kwForLoad(load){
  ...
  if (load.inputType === 'A') {
    const kva = isThreePhase()
      ? (Math.sqrt(3) * voltage * value / 1000)
      : (voltage * value / 1000);
...
function getCorrectionBreakdown(){
  ...
  const installation = INSTALL_COEF[state.installationMethod] || {
    air: null,
    rack: null,
    pipe: null
  };
...
function calcVoltageDrop(current, resistance, lengthM){
  ...
  if (!current || !resistance || !lengthM || !voltage) {
    return {
      dropV: 0,
      dropPercent: 0
    };
  ...
  const dropV = isThreePhase()
    ? (Math.sqrt(3) * current * resistance * lengthM / 1000)
    : (2 * current * resistance * lengthM / 1000);
...
function primaryFactor(candidate){
  ...
  const ratioCurrent = candidate.corrected
    ? sizingCurrentBase() / candidate.corrected
    : 0;
...
function allSelectionReasons(candidate){
  ...
  if (candidate.drop.dropPercent >= DROP_LIMIT_PERCENT * 0.7) {
    reasons.push('電圧降下');
  }

  if (state.calcMode === 'existing') {
    reasons.push('既設開閉器条件');
  }
...
function constructionNotes(totalMass){
  ...
  if (totalMass > 100) {
    notes.push('概算総質量が大きいため、支持条件を確認してください。');
  }

  if (totalMass > 300) {
    notes.push('ラック・支持材の耐荷重確認を推奨します。');
  }

  if (length > 100) {
    notes.push('長尺配線のため、施工計画に注意してください。');
  }

  if (!notes.length) {
    notes.push('概算質量は参考値です。最終的な施工計画は現場条件を確認してください。');
  }
...
function rootMemo(){
  ...
  if (state.ampacityMode === 'manual') {
    notes.push('基準許容電流は手動入力値を採用しています。');
  }
...
function missingFields(){
  ...
  if (state.calcMode === 'existing') {
    pushIf(!state.existingBreaker, '既設開閉器を選択してください。', 'existingBreaker');
  }
...
function calculate(){
  ...
  if (missing.length) {
    ...
    updateResultError(
      `未入力項目がある為、計算出来ません。入力して下さい。<br>${
        missing.map(v => '・' + escapeHtml(v.label)).join('<br>')
      }`
    );
  ...
  const drop = candidate
    ? calcVoltageDrop(
        ...
        Number(candidate.info?.resistance || 0),
        Number(state.wiringLength || 0)
      )
  ...
  if (!currentOk) {
    failReasons.push(
      `許容電流不足（必要 ${formatNumber(sizingCurrentBase(), 1)}A / 補正後許容 ${formatNumber(corrected, 1)}A）`
    );
  }

  if (!breakerOk) {
    failReasons.push(
      `開閉器容量不足（必要 ${requiredBreaker}A / 採用 ${adopted}A）`
    );
  }

  if (!dropOk) {
    failReasons.push(
      `電圧降下超過（${formatNumber(drop.dropPercent, 2)}% / 許容 ${DROP_LIMIT_PERCENT.toFixed(1)}%）`
    );
  }
...
function statusSpan(text){
  if (text === '適合' || text === '良') {
    return `<span class="status-ok">${text}</span>`;
  }

  if (text === '否') {
    return `<span class="status-ng">${text}</span>`;
  }
...
function updateLoadDerivedValues(load){
  ...
  if (typeLabel) {
    typeLabel.textContent = load.inputType || '-';
  }

  if (currentEl) {
    currentEl.textContent = formatNumber(currentForLoad(load), 2);
  }

  if (kwEl) {
    kwEl.textContent = formatNumber(kwForLoad(load), 3);
  }

  if (kvaEl) {
    kvaEl.textContent = formatNumber(kvaForLoad(load), 3);
  }
}
...
function buildFormulaCoef(){
  return textDoc(
    ...
    [
      ...
      ['条数補正', '1〜6条の係数テーブルを使用'],
      ['敷設方法補正', '気中・ラック・配管の選択値を使用'],
      ['敷設条件補正', '一般・密集・日射ありの選択値を使用'],
...
function buildFlexibleConduitDoc(){
  return buildConduitPayload(
    FLEXIBLE_CONDUIT,
    '可とう管',
    '金属製可とう電線管の非防水・防水の参考寸法表です。アプリ内では一般名称で表示します。'
  );
}
...
function buildDataIssuesDoc(){
  return {
    ...
    headers: ['No', '内容'],
    keys: ['no', 'text'],
    rows: DATA_ISSUES.map((text, index) => ({
      no: index + 1,
      text
    }))
  };
...
function buildConduitSupportDoc(){
  return textDoc(
    ...
    [
      ...
      ['支持方法', 'サドル・ハンガー・ラック・吊り材等の強度と固定条件を確認してください。'],
...
function buildRackWidthDoc(){
  return textDoc(
    ...
    [
      ...
      ['標準幅', 'W200 / W300 / W400 / W500 / W600 / W800 の範囲で表示します。'],
...
function buildRackSupportDoc(){
  return textDoc(
    ...
    [
      ...
      ['荷重', 'ケーブル質量・ラック自重・付属品・将来増設分を考慮してください。'],
...
function buildRackSeismicDoc(){
  return textDoc(
    ...
    [
      ...
      ['施工メモ', '長尺ラックや重量物周辺では、振れ止め・変位吸収・支持点の確認が重要です。'],
    ...
    rows: [
      {
        symbol: 'PAS',
        name: '気中負荷開閉器',
        body: '高圧引込部などに設置される開閉器。'
      },
      {
        symbol: 'DS',
        name: '断路器',
        body: '無負荷状態で回路を切り離す機器。'
      },
      {
        symbol: 'LBS',
        name: '高圧交流負荷開閉器',
        body: '負荷電流の開閉に用いられる機器。'
      },
      {
        symbol: 'VCB',
        name: '真空遮断器',
        body: '短絡電流等の遮断に用いられる遮断器。'
      },
      {
        symbol: 'CT',
        name: '計器用変流器',
        body: '電流計測・保護継電器入力に用いる。'
      },
      {
        symbol: 'VT',
        name: '計器用変圧器',
        body: '電圧計測・保護継電器入力に用いる。'
      },
      {
        symbol: 'LA',
        name: '避雷器',
  ...
  DOC_GROUPS.forEach(group => {
    ...
    group.items.forEach(item => {
      ...
      const headers = payload.headers
        .map(v => `<th>${escapeHtml(v)}</th>`)
        .join('');

      const keys =
        payload.keys ||
        (payload.rows[0] ? Object.keys(payload.rows[0]) : []);
...
function buildSaveCard(item, isHistory){
  ...
  card.innerHTML = `
    ...
    <div class="inline-actions top-gap">
      ...
      ${
        isHistory
          ? '<button class="ghost small js-save-history">保存済みに追加</button>'
          : `<button class="ghost small js-compare">${selected ? '比較から外す' : '比較に追加'}</button>`
      }
  ...
  card.querySelector('.js-open').onclick = () => {
    openSavedItem(item);
  };
...
function upsertSavedItem(item){
  ...
  if (savedItems.length > MAX_SAVED_ITEMS) {
    const removed = savedItems.slice(MAX_SAVED_ITEMS);
    const removedIds = new Set(removed.map(v => v.id));

    savedItems = savedItems.slice(0, MAX_SAVED_ITEMS);
    compareIds = compareIds.filter(id => !removedIds.has(id));
  }
}
...
function renderSaved(){
  ...
  if (!historyItems.length) {
    ...
    historyItems.forEach(item => {
      historyRoot.appendChild(buildSaveCard(item, true));
    });
  }
  ...
  const list = savedItems.filter(item => {
    const text = `${item.title || ''} ${item.memo || ''}`.toLowerCase();
  ...
  if (!list.length) {
    ...
    list.forEach(item => {
      savedRoot.appendChild(buildSaveCard(item, false));
    });
  }
...
function renderCompare(){
  ...
  const baseItem =
    compareItems.find(v => v.id === state.compareBaseId) ||
    compareItems[0];

  if (!state.compareBaseId) {
    state.compareBaseId = baseItem.id;
  }
  ...
  const metrics = [
    [
      '採用ケーブル',
      `${baseItem.result?.cableType || '-'} ${baseItem.result?.cableSize ? `${baseItem.result.cableSize}sq` : '-'}`,
      item => `${item.result?.cableType || '-'} ${item.result?.cableSize ? item.result.cableSize + 'sq' : '-'}`,
      null
    ],
    [
      '合計電流 [A]',
      baseItem.result?.totalCurrent,
      item => item.result?.totalCurrent,
      'number'
    ],
    [
      '電圧降下 [%]',
      baseItem.result?.voltageDropPercent,
      item => item.result?.voltageDropPercent,
      'number'
    ],
    [
      '開閉器裕度 [%]',
      baseItem.result?.breakerMarginPercent,
      item => item.result?.breakerMarginPercent,
      'number'
    ],
    [
      '容量裕度 [kW]',
      baseItem.result?.capacityMarginKW,
      item => item.result?.capacityMarginKW,
      'number'
    ],
    [
      '判定',
      baseItem.result?.judgement,
      item => item.result?.judgement,
      null
    ]
  ];
      ...
      if (type === 'number') {
        ...
        return `
          <td>
            ...
            <br>
            <span class="muted">差分 ${diff > 0 ? '+' : ''}${formatNumber(diff, 2)}</span>
          </td>
        `;
      }
