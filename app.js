...
function buildConduitPayload(dataset, groupName, note){
  const sections = Object.entries(dataset || {}).map(([type,items])=>{
    ...
    return {
      title:type,
      count:rows.length,
      note:`${groupName} / ${type} の参考寸法です。表内では配管種類名を繰り返さず、呼び・外径・内径等だけを表示します。`,
  ...
  return {
    note:`${note} 配管種類ごとに折りたたみ表示します。種類名をタップするとサイズ表を開きます。`,
    sections,
    headers:['配管種類','件数'],
    keys:['title','count'],
...
function buildConduitSupportDoc(){
  return {
    note:'配管支持間隔の参考表です。公共建築工事標準仕様書・標準施工要領等で一般に用いられる値を参考表示しています。建物仕様、耐震仕様、屋外・振動・曲がり部・ボックス付近では追加支持を確認してください。',
    headers:['対象','標準支持間隔','端部・曲がり部の注意','主な確認事項'],
    keys:['target','interval','edge','note'],
    rows:[
      {target:'金属管（厚鋼・薄鋼・ねじなし鋼製電線管）',interval:'2.0m以下',edge:'ボックス、器具、曲がり部、分岐部付近は必要に応じ支持',note:'屋外、振動部、立上り、重量物支持部は現場仕様を優先。'},
      {target:'合成樹脂管（CD管・PF管・VE管等）',interval:'1.5m以下',edge:'端部、曲がり部、ボックス付近はたわみ・抜けを確認',note:'熱伸縮、屋外紫外線、埋設/露出条件を確認。'},
      {target:'金属製可とう電線管',interval:'1.0m以下を目安',edge:'接続部付近・機器振動部は緩み止め、曲げ半径を確認',note:'防水形/非防水形、振動機器、可動部の余長を確認。'},
      {target:'FEP管・地中管路',interval:'埋設条件・管路設計による',edge:'ハンドホール、立上り、曲がり部、管口処理を確認',note:'土被り、埋戻し、曲げ、管路勾配、通線張力を別途確認。'},
      {target:'共通',interval:'支持間隔だけでなく支持強度で確認',edge:'管端・盤入線部・貫通部・プルボックス前後を確認',note:'標準値は参考。設計図、仕様書、内線規程、施工要領を優先。'}
    ]
  };
}
...
function buildGroundWireDoc(){ return {note:'主幹開閉器容量からの接地線サイズ参考表です。接地工事種別はA/B/C/D種で整理し、EA/EB/EC/EDは図面・端子記号として扱います。ELBは接地種別ではなく遮断条件として別確認してください。', headers:['接地種別','電線種類','最大開閉器 [A]','参考サイズ'], keys:['groundType','wireType','maxBreaker','size'], rows:GROUND_RULES}; }
  ...
  visibleGroups.forEach(({group, visibleItems})=>{
    ...
    visibleItems.forEach(entry=>{
      ...
      if (Array.isArray(payload.sections) && payload.sections.length) {
        ...
        if (keys.includes('body') || keys.includes('note') || isTextHeavy) table.classList.add('doc-text-table');
        if (isTextHeavy) { table.classList.add('doc-card-table'); wrap.classList.add('doc-card-table-wrap'); }
...
function bindEvents(){
  ...
  document.querySelectorAll('.seg').forEach(btn=>btn.addEventListener('click',()=>{ document.querySelectorAll('.seg').forEach(v=>v.classList.remove('active')); btn.classList.add('active'); state.calculationType = btn.dataset.calcType; normalizePowerSystemForCalcType(); normalizeCableTypeForPowerSystem(); persistState(); renderAll(); }));
...
function updateGroundResult(){
  const groundType = $('groundType').value, wireType = $('groundWireType').value, breaker = Number(state.lastResult?.cableSelectionBasisCurrent || state.lastResult?.adoptedBreaker || state.existingBreaker || 0);
  ...
  $('groundResult').textContent = rule ? `基準開閉器 ${breaker || '-'}A の参考接地線サイズ：${rule.size}` : '条件に合う参考サイズがありません。';
}
