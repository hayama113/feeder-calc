...
function rootMemo(){
  ...
  notes.push('接地種別は入力電圧からC種（EC）/D種（ED）等を推定し、接地線サイズは各選定ページの開閉器容量を基準に参考表示します。');
...
function cableMetrics(candidate){
  ...
  const massKgM = info?.massKgKm ? Number(info.massKgKm) / 1000 : 0;
  const wiringLength = Number(state.wiringLength || 0);
  const parallel = Number(state.parallelCount || 1);
  const massTotalKg = massKgM && wiringLength ? massKgM * wiringLength * parallel : 0;
  const rackWidth = info?.outerDiameter ? rackWidthFor(Number(info.outerDiameter), parallel) : '-';
  return { massKgM, massTotalKg, rackWidth };
}

function groundKindForVoltage(voltage){
  const v = Number(voltage || 0);
  if (!v) return {kind:'', symbol:'', label:'-', resistance:'-', autoTrip:'-', note:'電圧未入力のため接地種別を推定できません。'};
  if (v <= 300) return {kind:'D種', symbol:'ED', label:'D種接地工事（ED）', resistance:'100Ω以下', autoTrip:'0.5秒以内自動遮断時：500Ω以下可', note:'300V以下の低圧機械器具外箱等の代表選定です。'};
  if (v <= 600) return {kind:'C種', symbol:'EC', label:'C種接地工事（EC）', resistance:'10Ω以下', autoTrip:'0.5秒以内自動遮断時：500Ω以下可', note:'300V超過の低圧機械器具外箱等の代表選定です。'};
  return {kind:'A種', symbol:'EA', label:'A種接地工事（EA）又は高圧側要確認', resistance:'10Ω以下目安', autoTrip:'設備区分により要確認', note:'低圧範囲外の可能性があるため、高圧/特別高圧の設備区分で確認してください。'};
}
