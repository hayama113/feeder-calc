    ...
    visibleItems.forEach(entry=>{
      ...
      if (Array.isArray(payload.sections) && payload.sections.length) {
        ...
        payload.sections.forEach((section, sectionIndex)=>{
          ...
          const headers = (section.headers || []).map(v=>`<th>${escapeHtml(v)}</th>`).join('');
          const rows = (section.rows || []).map(row=>`<tr>${sectionKeys.map(key=>`<td>${escapeHtml(row[key] ?? '-')}</td>`).join('')}</tr>`).join('');
