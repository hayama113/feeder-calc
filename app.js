...
function calculate(options={}){
  ...
  const result = {
    ...
    cableSizingMode,
    basisCableType:selectedCableTypeFromState(), basisCableSize:basisCandidate?.size || '', basisCableCorrectedAmpacity:basisCandidate?.corrected || 0, basisCableVoltageDropV:basisCandidate?.drop?.dropV || 0, basisCableVoltageDropPercent:basisCandidate?.drop?.dropPercent || 0, basisCableDropBasisCurrent:basisCandidate?.dropBasisCurrent || 0, basisCableMassKgM:basisMetrics.massKgM, basisCableMassTotalKg:basisMetrics.massTotalKg, basisCableRackWidth:basisMetrics.rackWidth,
    requiredMinCableType:selectedCableTypeFromState(), requiredMinCableSize:requiredCandidate?.size || '', requiredMinCableCorrectedAmpacity:requiredCandidate?.corrected || 0, requiredMinCableVoltageDropV:requiredCandidate?.drop?.dropV || 0, requiredMinCableVoltageDropPercent:requiredCandidate?.drop?.dropPercent || 0, requiredMinCableDropBasisCurrent:requiredCandidate?.dropBasisCurrent || 0, requiredMinCableMassKgM:requiredMetrics.massKgM, requiredMinCableMassTotalKg:requiredMetrics.massTotalKg, requiredMinCableRackWidth:requiredMetrics.rackWidth,
...
function applyResultPageOrder(result){
  ...
  if (isExisting) {
    if (existingCard) {
      ...
      existingCard.style.order = '1';
    }
    if (recCard) recCard.style.order = '2';
    if (minCard) minCard.style.order = '3';
    setTextIfExists('resExistingPageBadge', '1 / 3');
    setTextIfExists('resRecPageBadge', '2 / 3');
    setTextIfExists('resMinPageBadge', '3 / 3');
    if (orderLabel) orderLabel.textContent = '横スクロール：既設選定 → 推奨選定 → 最小選定';
  } else {
    if (existingCard) existingCard.classList.add('hidden');
    if (recCard) recCard.style.order = '1';
    if (minCard) minCard.style.order = '2';
    setTextIfExists('resRecPageBadge', '1 / 2');
    setTextIfExists('resMinPageBadge', '2 / 2');
    if (orderLabel) orderLabel.textContent = '横スクロール：推奨選定 → 最小選定';
  }

  if (pages) {
    pages.setAttribute('aria-label', isExisting ? '既設・推奨・最小の選定結果' : '推奨・最小の選定結果');
    if (pages.dataset.resultMode !== (isExisting ? 'existing' : 'auto')) {
      pages.dataset.resultMode = isExisting ? 'existing' : 'auto';
